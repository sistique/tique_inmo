...
use gamboamartin\inmuebles\html\inm_doc_factura_compra_html;
...
class controlador_inm_doc_factura_compra extends _ctl_formato {

    public string $ruta_doc = '';
    public bool $es_imagen = false;
    public bool $es_pdf = false;

    public string $button_inm_doc_factura_compra_descarga = '';
    public function __construct(PDO      $link, html $html = new \gamboamartin\template_1\html(),
                                stdClass $paths_conf = new stdClass())
    {
        $modelo = new inm_doc_factura_compra(link: $link);
        $html_ = new inm_doc_factura_compra_html(html: $html);
    ...
    public function alta(bool $header, bool $ws = false): array|string
    {
        ...
        $columns_ds = array('inm_factura_compra_descripcion');
        ...
        $inm_factura_compra = (new inm_factura_compra(link: $this->link))->registro(registro_id: $this->registro_id);
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al obtener inm_factura_compra', data: $inm_factura_compra,
                header: $header, ws: $ws);
        }
    ...
    public function elimina_temporal(bool $header, bool $ws = false){
        ...
        $modelo_inm_doc_prospecto = new inm_doc_factura_compra(link: $this->link);
    ...
    {
        $name = $registro->inm_factura_compra_id.".".$registro->inm_factura_compra_nombre;
        $name .= ".".$registro->inm_factura_compra_apellido_paterno;
        $name .= ".".$registro->inm_factura_compra_apellido_materno.".".$registro->doc_tipo_documento_codigo;
        return $name;
    }
    ...
    {
        $columns["inm_doc_factura_compra_id"]["titulo"] = "Id";
        ...
        $columns["inm_factura_compra_nombre"]["titulo"] = "Nombre";
        $columns["inm_factura_compra_apellido_paterno"]["titulo"] = "AP";
        $columns["inm_factura_compra_apellido_materno"]["titulo"] = "AM";

        $filtro = array("inm_doc_factura_compra.id","doc_documento.id", "doc_documento.ruta_relativa",
            'doc_tipo_documento.descripcion','inm_factura_compra.nombre','inm_factura_compra.apellido_paterno',
            'inm_factura_compra_apellido_materno');
    ...
    public function vista_previa(bool $header, bool $ws = false): array|string|stdClass
    {
        ...
        $row_upd->nss = $registro->inm_factura_compra_nss;
        $row_upd->curp = $registro->inm_factura_compra_curp;
        $row_upd->apellido_paterno = $registro->inm_factura_compra_apellido_paterno;
        $row_upd->apellido_materno = $registro->inm_factura_compra_apellido_materno;
        $row_upd->nombre = $registro->inm_factura_compra_nombre;
        ...
        $button_inm_doc_factura_compra_descarga = $this->html->button_href(accion: 'descarga',etiqueta:  'Descarga',
            registro_id:  $this->registro_id,
            seccion:  $this->seccion,style:  'success');
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al integrar button',data:  $button_inm_doc_factura_compra_descarga,
                header: $header,ws:  $ws);
        }


        $this->button_inm_doc_factura_compra_descarga = $button_inm_doc_factura_compra_descarga;

        $inm_doc_factura_compra_id = $this->html->hidden(name:'inm_doc_factura_compra_id',value: $this->registro_id);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al in_registro_id',data:  $inm_doc_factura_compra_id,
                header: $header,ws:  $ws);
        }
        $this->inputs->inm_doc_factura_compra_id = $inm_doc_factura_compra_id;
