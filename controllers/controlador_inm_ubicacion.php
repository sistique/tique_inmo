...
class controlador_inm_ubicacion extends _ctl_base
{
    ...
    public string $descripcion_poliza_secundaria_firmada = '';
    public string $button_inm_doc_ubicacion_descarga_firmado_poliza_secundaria = '';
    public string $button_inm_doc_ubicacion_vista_previa_firmado_poliza_secundaria = '';
    ...
    public function asigna_firmado(bool $header, bool $ws = false): array|stdClass
    {
        ...
        if ($r_inm_doc_ubicacion->n_registros > 0) {
            ...
            $this->button_inm_doc_ubicacion_descarga_transferencia_firmada = $button_inm_doc_ubicacion_descarga;
            ...
            $this->button_inm_doc_ubicacion_vista_previa_transferencia_firmada = $button_inm_doc_ubicacion_vista_previa;
            ...
            $this->button_inm_doc_ubicacion_elimina_bd_transferencia_firmada = $button_inm_doc_ubicacion_elimina_bd;
        }
        ...
        if ($r_inm_doc_ubicacion->n_registros > 0) {
            ...
            $this->button_inm_doc_ubicacion_descarga_firmado_poliza_secundaria = $button_inm_doc_ubicacion_descarga;
            ...
            $this->button_inm_doc_ubicacion_vista_previa_firmado_poliza_secundaria = $button_inm_doc_ubicacion_vista_previa;
            ...
            $this->button_inm_doc_ubicacion_elimina_bd_firmado_poliza_secundaria = $button_inm_doc_ubicacion_elimina_bd;
        }
        return $base;
    }
    ...
    public function asigna_emision_de_recurso(bool $header, bool $ws = false): array|stdClass
    {
        ...
        foreach ($r_inm_efectivo->registros as $inm_efectivo) {
            ...
            if ($r_rel_doc_efectivo->n_registros > 0) {
                ...
                $button_vista_previa = $this->html->button_href(accion: 'vista_previa',
                    etiqueta: 'Vista Previa', registro_id: $r_rel_doc_efectivo->registros[0]['inm_doc_ubicacion_id'],
                    seccion: 'inm_doc_ubicacion', style: 'success');
                if (errores::$error) {
                    return $this->retorno_error(mensaje: 'Error al integrar button',
                        data: $button_vista_previa, header: $header, ws: $ws);
                ...
                $button_descarga_zip = $this->html->button_href(accion: 'descarga_zip',
                    etiqueta: 'Descarga ZIP', registro_id: $r_rel_doc_efectivo->registros[0]['inm_doc_ubicacion_id'],
                    seccion: 'inm_doc_ubicacion', style: 'success');
                if (errores::$error) {
                    return $this->retorno_error(mensaje: 'Error al integrar button',
                        data: $button_descarga_zip, header: $header, ws: $ws);
    ...
    public function descarga_fotos(bool $header, bool $ws = false){
        $comprime = $this->descarga_archivos(es_foto: 'activo', prefijo_zip: 'FOTOS', header: $header, ws: $ws);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener documento',data:  $comprime,header:  $header,
                ws:  $ws);
        }
    ...
    public function descarga_archivos(string $es_foto, string $prefijo_zip, bool $header, bool $ws = false){
        ...
        $filtro['inm_conf_docs_ubicacion.es_foto'] = $es_foto;
        ...
        foreach ($inm_conf_docs_prospecto->registros as $conf_doc) {
            ...
            $docs = (new inm_doc_ubicacion(link: $this->link))->filtro_and(filtro: $filtro_doc);
            if(errores::$error){
...
*/
    ...
    public function reparacion(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $fecha = $this->html->input_fecha(cols: 6, row_upd: new stdClass(), value_vacio: false, name: 'fecha_inicio',
            place_holder: 'Fecha Inicio', value: $hoy, value_hora: true);
        if (errores::$error) {
            $this->retorno_error(mensaje: 'Error al generar input fecha', data: $fecha, header: $header, ws: $ws);
        }

        $this->inputs->fecha_reparacion = $fecha;
        ...
        $link_alta_reparacion = $this->obj_link->link_alta_bd(link: $this->link, seccion: 'inm_reparacion',
            params: $params);
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al generar link', data: $link_alta_reparacion, header: $header,
                ws: $ws);
        }

        $this->link_alta_reparacion = $link_alta_reparacion;
    ...
    public function llaves(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $link_alta_llave = $this->obj_link->link_alta_bd(link: $this->link, seccion: 'inm_llave',
            params: $params);
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al generar link', data: $link_alta_llave, header: $header,
                ws: $ws);
        }
    ...
    public function firmado_bd(bool $header, bool $ws = false)
    {
        ...
        $filtro_exi['inm_ubicacion.id'] = $this->registro_id;
        $filtro_exi['inm_status_ubicacion.id'] = 7;
        $existe = (new inm_bitacora_status_ubicacion(link: $this->link))->existe(filtro: $filtro_exi);
        if (errores::$error) {
            $this->link->rollBack();
            return $this->retorno_error(mensaje: 'Error al obtener datos de bitacora', data: $existe,
        ...
        if (isset($_FILES['documentos_efectivos'])) {
            foreach ($_FILES['documentos_efectivos']['name'] as $costo => $tipo_documento) {
                foreach ($tipo_documento as $key => $value) {
                    foreach ($value as $nombre_documento) {
                        $tipos_documento_efectivo[$costo][$key]['name'] = $nombre_documento;
                    }
                }
            }

            foreach ($_FILES['documentos_efectivos']['tmp_name'] as $costo => $tipo_documento) {
                foreach ($tipo_documento as $key => $value) {
                    foreach ($value as $nombre_documento) {
                        $tipos_documento_efectivo[$costo][$key]['tmp_name'] = $nombre_documento;
                    }
                }
            }
        }
        ...
        foreach ($tipos_documento_efectivo as $efectivo_id => $doc) {
            ...
            foreach ($doc as $tipo_documento_id => $value) {
                ...
                if ($valor_documento['name'] !== '' && $valor_documento['tmp_name'] !== '') {
                    ...
                    $registro_rel = array();
                    $registro_rel['inm_efectivo_id'] = $efectivo_id;
                    $registro_rel['inm_doc_ubicacion_id'] = $result->registro_id;
                    $r_rel_doc_ubi_efe = (new inm_rel_doc_efectivo_ubicacion(link: $this->link))->alta_registro(registro: $registro_rel);
                    if (errores::$error) {
                        return $this->retorno_error(mensaje: 'Error al insertar datos', data: $r_rel_doc_ubi_efe,
                            header: $header, ws: $ws);
                    }
                }
            }
        ...
        $filtro_doc['inm_ubicacion.id'] = $this->registro_id;
        $filtro_doc['doc_tipo_documento.id'] = 48;
        $existe = (new inm_doc_ubicacion(link: $this->link))->existe(filtro: $filtro_doc);
        if (errores::$error) {
            $this->link->rollBack();
            return $this->retorno_error(mensaje: 'Error al obtener datos de bitacora', data: $existe,
        ...
        if(!$existe) {
            if(trim($_FILES['poliza_comision_firmada']['name']) !== '') {
                $_FILES['documento'] = $_FILES['poliza_comision_firmada'];
                $registro = array();
                $registro['inm_ubicacion_id'] = $this->registro_id;
                $registro['doc_tipo_documento_id'] = 48;
                $r_inm_doc_ubicacion = (new inm_doc_ubicacion(link: $this->link))->alta_registro(registro: $registro);
                if (errores::$error) {
                    $this->link->rollBack();
                    return $this->retorno_error(mensaje: 'Error al insertar datos', data: $r_inm_doc_ubicacion,
        ...
        $filtro_doc['inm_ubicacion.id'] = $this->registro_id;
        $filtro_doc['doc_tipo_documento.id'] = 49;
        $existe = (new inm_doc_ubicacion(link: $this->link))->existe(filtro: $filtro_doc);
        if (errores::$error) {
            $this->link->rollBack();
            return $this->retorno_error(mensaje: 'Error al obtener datos de bitacora', data: $existe,
        ...
        if(!$existe) {
            if(trim($_FILES['transferencia']['name']) !== '') {
                $_FILES['documento'] = $_FILES['transferencia'];
                $registro = array();
                $registro['inm_ubicacion_id'] = $this->registro_id;
                $registro['doc_tipo_documento_id'] = 49;
                $r_inm_doc_ubicacion = (new inm_doc_ubicacion(link: $this->link))->alta_registro(registro: $registro);
                if (errores::$error) {
                    $this->link->rollBack();
                    return $this->retorno_error(mensaje: 'Error al insertar datos', data: $r_inm_doc_ubicacion,
        ...
        $filtro_doc['inm_ubicacion.id'] = $this->registro_id;
        $filtro_doc['doc_tipo_documento.id'] = 50;
        $existe = (new inm_doc_ubicacion(link: $this->link))->existe(filtro: $filtro_doc);
        if (errores::$error) {
            $this->link->rollBack();
            return $this->retorno_error(mensaje: 'Error al obtener datos de bitacora', data: $existe,
        ...
        if(!$existe) {
            if(trim($_FILES['poliza_secundaria_firmada']['name']) !== '') {
                $_FILES['documento'] = $_FILES['poliza_secundaria_firmada'];
                $registro = array();
                $registro['inm_ubicacion_id'] = $this->registro_id;
                $registro['doc_tipo_documento_id'] = 50;
                $r_inm_doc_ubicacion = (new inm_doc_ubicacion(link: $this->link))->alta_registro(registro: $registro);
                if (errores::$error) {
                    $this->link->rollBack();
                    return $this->retorno_error(mensaje: 'Error al insertar datos', data: $r_inm_doc_ubicacion,
    ...
    protected function key_selects_txt(array $keys_selects): array
    {
        ...
        $keys_selects = (new init())->key_select_txt(cols: 6, key: 'calle_domicilio',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'nombre_notario',
            keys_selects: $keys_selects, place_holder: 'Nombre Notario', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'numero_notaria',
            keys_selects: $keys_selects, place_holder: 'Numero Notaria', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'plaza_notaria',
            keys_selects: $keys_selects, place_holder: 'Plaza Notaria', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'numero_escritura',
            keys_selects: $keys_selects, place_holder: 'Numero Escritura', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'libro',
            keys_selects: $keys_selects, place_holder: 'Libro', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'volumen',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'entrada',
            keys_selects: $keys_selects, place_holder: 'Entrada', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'supermanzana',
            keys_selects: $keys_selects, place_holder: 'Supermanzana', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'edificio',
            keys_selects: $keys_selects, place_holder: 'Edificio', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'condominio',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'adeudo_hipoteca',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'adeudo_predial',
            keys_selects: $keys_selects, place_holder: 'Adeudo Predial', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'cuenta_predial',
            keys_selects: $keys_selects, place_holder: 'Cuenta Predial', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'cuenta_agua',
            keys_selects: $keys_selects, place_holder: 'Cuenta Agua', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'adeudo_agua',
            keys_selects: $keys_selects, place_holder: 'Adeudo Agua', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'cuenta_luz',
            keys_selects: $keys_selects, place_holder: 'Cuente Luz', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'adeudo_luz',
            keys_selects: $keys_selects, place_holder: 'Adeudo Luz', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'monto_devolucion',
