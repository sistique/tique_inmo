...
class _ubicacion{
    ...
    private function aplica_seccion_co_acreditado(array $inm_ubicacion): bool
    {
    ...
    final public function frontend_co_acreditado(controlador_inm_ubicacion $controler, stdClass $row_upd = new stdClass()){
        $aplica_seccion_co_acreditado = $this->aplica_seccion_co_acreditado(inm_ubicacion: $controler->registro);
        if(errores::$error){
        ...
        $headers = $this->get_headers(inm_ubicacion: $controler->registro);
        if(errores::$error){
    ...
    private function get_headers(array $inm_ubicacion){
        ...
        $aplica_seccion_co_acreditado = $this->aplica_seccion_co_acreditado(inm_ubicacion: $inm_ubicacion);
        if(errores::$error){
    ...
    private function header_apartado(inm_ubicacion_html $html_entidad, int $n_apartado, string $tag_header){
        $param = $this->param_header(n_apartado: $n_apartado);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al generar parametros',data:  $param);
        }
