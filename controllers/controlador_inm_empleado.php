...
class controlador_inm_empleado extends _ctl_formato {
    ...
    public array $cuentas_bancarias = array();
    public string $link_alta_cuenta = '';
    public string $link_alta_conf_empleado = '';
    ...
    protected function campos_view(): array
    {
        ...
        $keys->inputs = array('descripcion','nss','rfc','nombre','apellido_paterno','apellido_materno','celular',
            ...
            'salario_diario_integrado','curp','num_cuenta','clabe');
        $keys->selects = array();
    ...
    public function genera_cuenta_bancaria(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $keys_selects = array();
        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(),
            key: 'bn_sucursal_id', keys_selects: $keys_selects, id_selected: -1,
            label: 'Banco');
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects);
        }
        ...
        $keys_selects = (new init())->key_select_txt(cols: 6,key: 'num_cuenta',
            keys_selects: $keys_selects, place_holder: 'Numero de Cuenta', required: false);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 6,key: 'clabe',
            keys_selects: $keys_selects, place_holder: 'CLABE', required: false);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects);
        }

        $r_rel_emp = (new inm_rel_emp_emp(link: $this->link))->filtro_and(filtro: array('inm_empleado.id' => $this->registro_id));
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $r_rel_emp);
        }
        ...
        if($r_rel_emp->n_registros > 0){
            $r_cuentas = (new em_cuenta_bancaria(link: $this->link))->filtro_and(
                filtro: array("em_empleado.id" => $r_rel_emp->registros[0]['em_empleado_id']));
            if (errores::$error) {
                return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $r_rel_emp);
            }

            $cuentas_bancarias = $r_cuentas->registros;
        }
        ...
        $link_alta_cuenta = $this->obj_link->link_alta_bd(link: $this->link, seccion: 'em_cuenta_bancaria');
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al generar link', data: $link_alta_cuenta, header: $header,
                ws: $ws);
        }

        $this->link_alta_cuenta = $link_alta_cuenta;

        return $base;
    }

    public function genera_conf_nomina(bool $header, bool $ws = false): array|stdClass
    {
        $r_modifica = $this->init_modifica(); // TODO: Change the autogenerated stub
        if (errores::$error) {
            return $this->retorno_error(
        ...
        $r_conf_emp = (new nom_conf_empleado(link: $this->link))->filtro_and(filtro:
            array('em_empleado.id' => $r_rel_emp->registros[0]['em_empleado_id']));
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $r_conf_emp);
        }
        ...
        $id_selected = -1;
        if($r_conf_emp->n_registros > 0){
            $id_selected = $r_conf_emp->registros[0]['em_cuenta_bancaria_id'];
        }
        ...
        $id_selected = $r_rel_emp->registros[0]['em_empleado_id'];
        $filtro['em_empleado.id'] = $r_rel_emp->registros[0]['em_empleado_id'];
        $keys_selects = $this->key_select(cols: 12, con_registros: true,filtro: $filtro,
            key: 'em_empleado_id', keys_selects: $keys_selects, id_selected: $id_selected,
            label: 'Empleado');
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects);
        ...
        $id_selected = -1;
        if($r_conf_emp->n_registros > 0){
            $id_selected = $r_conf_emp->registros[0]['nom_conf_nomina_id'];
        }
        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(),
        ...
        $link_conf_empleado = $this->obj_link->link_alta_bd(link: $this->link, seccion: 'nom_conf_empleado');
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al generar link', data: $link_conf_empleado, header: $header,
                ws: $ws);
        }

        $this->link_alta_conf_empleado = $link_conf_empleado;
