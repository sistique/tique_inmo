...
use gamboamartin\inmuebles\html\inm_doc_periodo_asistencia_html;
...
class controlador_inm_doc_periodo_asistencia extends _ctl_formato {

    public string $ruta_doc = '';
    public bool $es_imagen = false;
    public bool $es_pdf = false;

    public string $button_inm_doc_periodo_asistencia_descarga = '';
    public function __construct(PDO      $link, html $html = new \gamboamartin\template_1\html(),
                                stdClass $paths_conf = new stdClass())
    {
        $modelo = new inm_doc_periodo_asistencia(link: $link);
        $html_ = new inm_doc_periodo_asistencia_html(html: $html);
    ...
    public function alta(bool $header, bool $ws = false): array|string
    {
        ...
        $columns_ds = array('inm_periodo_asistencia_descripcion');
        $keys_selects = $this->key_select(cols:12, con_registros: true,filtro:  array(), key: 'inm_periodo_asistencia_id',
    ...
    public function elimina_temporal(bool $header, bool $ws = false){
        ...
        $modelo_inm_doc_prospecto = new inm_doc_periodo_asistencia(link: $this->link);
    ...
    {
        $name = $registro->inm_periodo_asistencia_id.".".$registro->inm_periodo_asistencia_nombre;
        $name .= ".".$registro->inm_periodo_asistencia_apellido_paterno;
        $name .= ".".$registro->inm_periodo_asistencia_apellido_materno.".".$registro->doc_tipo_documento_codigo;
        return $name;
    }
    ...
    {
        $columns["inm_doc_periodo_asistencia_id"]["titulo"] = "Id";
        ...
        $columns["inm_periodo_asistencia_nombre"]["titulo"] = "Nombre";
        $columns["inm_periodo_asistencia_apellido_paterno"]["titulo"] = "AP";
        $columns["inm_periodo_asistencia_apellido_materno"]["titulo"] = "AM";

        $filtro = array("inm_doc_periodo_asistencia.id","doc_documento.id", "doc_documento.ruta_relativa",
            'doc_tipo_documento.descripcion','inm_periodo_asistencia.nombre','inm_periodo_asistencia.apellido_paterno',
            'inm_periodo_asistencia_apellido_materno');
    ...
    public function vista_previa(bool $header, bool $ws = false): array|string|stdClass
    {
        ...
        $row_upd->nss = $registro->inm_periodo_asistencia_nss;
        $row_upd->curp = $registro->inm_periodo_asistencia_curp;
        $row_upd->apellido_paterno = $registro->inm_periodo_asistencia_apellido_paterno;
        $row_upd->apellido_materno = $registro->inm_periodo_asistencia_apellido_materno;
        $row_upd->nombre = $registro->inm_periodo_asistencia_nombre;
        ...
        $button_inm_doc_periodo_asistencia_descarga = $this->html->button_href(accion: 'descarga',etiqueta:  'Descarga',
            registro_id:  $this->registro_id,
            seccion:  $this->seccion,style:  'success');
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al integrar button',data:  $button_inm_doc_periodo_asistencia_descarga,
                header: $header,ws:  $ws);
        }


        $this->button_inm_doc_periodo_asistencia_descarga = $button_inm_doc_periodo_asistencia_descarga;

        $inm_doc_periodo_asistencia_id = $this->html->hidden(name:'inm_doc_periodo_asistencia_id',value: $this->registro_id);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al in_registro_id',data:  $inm_doc_periodo_asistencia_id,
                header: $header,ws:  $ws);
        }
        $this->inputs->inm_doc_periodo_asistencia_id = $inm_doc_periodo_asistencia_id;
