...
class controlador_inm_beneficiario extends _ctl_base {
    ...
    public function modifica(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $columns_ds = array('inm_prospecto_nombre','inm_prospecto_apellido_paterno','inm_prospecto_apellido_materno');
        $keys_selects = $this->key_select(cols:12, con_registros: true,filtro:  array(), key: 'inm_prospecto_id',
            keys_selects: array(), id_selected: $this->row_upd->inm_prospecto_id,
            label: 'Prospecto',columns_ds: $columns_ds, disabled: true,required: false);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects, header: $header,ws:  $ws);
        }
        ...
        $keys_selects = $this->key_select(cols: 12, con_registros: true, filtro: array(), key: 'inm_comprador_id',
            keys_selects: $keys_selects, id_selected: $this->row_upd->inm_comprador_id,
            label: 'Comprador', columns_ds: $columns_ds, disabled: true, required: false);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects, header: $header,ws:  $ws);
        }

        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(), key: 'inm_tipo_beneficiario_id',
            keys_selects: $keys_selects, id_selected: $this->row_upd->inm_tipo_beneficiario_id,
        ...
        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(), key: 'inm_parentesco_id',
            keys_selects: $keys_selects, id_selected: $this->row_upd->inm_parentesco_id,
            label: 'Parenteso');
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects, header: $header,ws:  $ws);
        }
