...
class controlador_inm_prospecto_ubicacion extends _ctl_formato
{
    ...
    public function alta(bool $header, bool $ws = false): array|string
    {
        ...
        $keys_selects = $this->key_select(cols:4, con_registros: $con_registros,filtro: $filtro_agente_sel,
            key: 'com_agente_id', keys_selects:$keys_selects, id_selected: $com_agente_id, label: 'Agente');
        if(errores::$error){
        ...
        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'metros_terreno',
            keys_selects: $keys_selects, place_holder: 'Metros Terreno', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'metros_construccion',
    ...
    public function descarga_archivos(string $es_foto, string $prefijo_zip, bool $header, bool $ws = false){
        ...
        $filtro['inm_conf_docs_prospecto_ubicacion.es_foto'] = $es_foto;
        ...
        foreach ($inm_conf_docs_prospecto->registros as $conf_doc) {
            ...
            $docs = (new inm_doc_prospecto_ubicacion(link: $this->link))->filtro_and(filtro: $filtro_doc);
            if(errores::$error){
...
*/
    ...
    public function integra_relacion(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $inm_prospecto_ubicacion_ubicacion = $this->html->input_text(cols: 12, disabled: true,
            name: 'inm_prospecto_ubicacion_ubicacion', place_holder: 'Ubicacion', row_upd: $this->row_upd,
            value_vacio: false, required: false);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener input',data:  $inm_prospecto_ubicacion_ubicacion,
                header: $header, ws: $ws);
        }

        $this->inputs->inm_prospecto_ubicacion_ubicacion = $inm_prospecto_ubicacion_ubicacion;
    ...
    protected function key_selects_txt(array $keys_selects, int $cols_descripcion = 12): array
    {
        ...
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'nombre',
            keys_selects: $keys_selects, place_holder: 'Nombre');
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'apellido_paterno',
            keys_selects: $keys_selects, place_holder: 'Apellido Paterno');
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'apellido_materno',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'calle',
        ...
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'numero_exterior_domicilio',
            keys_selects: $keys_selects, place_holder: 'No. Ext Domicilio', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'numero_interior_domicilio',
            keys_selects: $keys_selects, place_holder: 'No. Int Domicilio', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        ...
        $keys_selects = (new init())->key_select_txt(cols: 2, key: 'cuenta_luz',
            keys_selects: $keys_selects, place_holder: 'Cuenta Luz', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        ...
        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'numero_notaria',
            keys_selects: $keys_selects, place_holder: 'No. Notaria', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        ...
        $keys_selects = (new init())->key_select_txt(cols: 4, key: 'numero_escritura',
            keys_selects: $keys_selects, place_holder: 'No. Escritura', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }
        ...
        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'manzana',
            keys_selects: $keys_selects, place_holder: 'Manzana', required: false);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar key_selects', data: $keys_selects);
        }

        $keys_selects = (new init())->key_select_txt(cols: 3, key: 'lote',
    ...
    public function modifica_bd(bool $header, bool $ws): array|stdClass
    {
        ...
        $result_co_acreditado = $this->modelo->transacciona_co_acreditado(inm_prospecto_ubicacion_id: $this->registro_id,
            link: $this->link);
        if (errores::$error) {
            $this->link->rollBack();
    ...
    public function proceso_prospecto_ubicacion(bool $header, bool $ws = false): array|stdClass
    {
        $r_modifica = $this->init_modifica(); // TODO: Change the autogenerated stub
        if(errores::$error){
            return $this->retorno_error(
        ...
        $btn_action_next = $this->html->hidden('btn_action_next', value: 'proceso_prospecto_ubicacion');
        if (errores::$error) {
            return $this->retorno_error(
