...
class com_email_cte extends _modelo_parent{
    ...
    public function __construct(PDO $link, array $childrens = array()){
        ...
        $tipo_campos['descripcion'] = 'correo';
    ...
    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {
        ...
        $filtro['com_email_cte.descripcion'] = $this->registro['descripcion'];
        $existe = (new com_email_cte(link: $this->link))->existe(filtro: $filtro);
        if(errores::$error){
        ...
        if($existe){
            ...
            if(errores::$error){
                return $this->error->error(mensaje: 'Error al obtener registro',data:  $r_com_email_cte);
            }
            if($r_com_email_cte->n_registros === 0){
                return $this->error->error(mensaje: 'Error no existe registro',data:  $r_com_email_cte);
            }
            if($r_com_email_cte->n_registros > 1){
                return $this->error->error(mensaje: 'Error existe mas de un registro',data:  $r_com_email_cte);
            }

            $this->registro_id = $r_com_email_cte->registros[0]['com_email_cte_id'];
            ...
            $r_alta_bd->registro = $r_com_email_cte->registros[0];
            $r_alta_bd->registro_obj = (object)$r_com_email_cte->registros[0];
