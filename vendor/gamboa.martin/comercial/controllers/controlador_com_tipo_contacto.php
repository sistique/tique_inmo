...
use html\com_tipo_contacto_html;
use PDO;
use stdClass;

class controlador_com_tipo_contacto extends _base_sin_cod {

    public array|stdClass $keys_selects = array();
    public controlador_com_tipo_contacto $com_tipo_contacto;

    public string $link_com_contacto_alta_bd = '';
    ...
    public function __construct(PDO $link, html $html = new \gamboamartin\template_1\html(),
        ...
        $this->childrens_data['com_contacto']['title'] = 'Contactos';
    ...
    public function init_datatable(): stdClass
    {
        ...
        $datatables->filtro[] = 'com_tipo_contacto.id';
    ...
    private function init_links(): array|string
    {
        ...
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al generar links para tipo contacto',data:  $this->obj_link);
        }

        $this->link_com_contacto_alta_bd = $this->obj_link->link_alta_bd(link: $this->link, seccion: 'com_tipo_contacto');
        if(errores::$error){
            $error = $this->errores->error(mensaje: 'Error al obtener link',data:  $this->link_com_contacto_alta_bd);
            print_r($error);
            exit;
        }

        return $this->link_com_contacto_alta_bd;
    }
    ...
    protected function key_selects_txt(array $keys_selects): array
    {
        ...
        $keys_selects = (new \base\controller\init())->key_select_txt(cols: 8,key: 'descripcion',
            keys_selects:$keys_selects, place_holder: 'Tipo Contacto');
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects);
        }
