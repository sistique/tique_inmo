...
class bn_sucursal_html extends html_controler {
    ...
    public function select_bn_sucursal_id(int $cols, bool $con_registros, int $id_selected, PDO $link,
                                          bool $disabled = false): array|string
    {
        $modelo = new bn_sucursal(link: $link);

        $select = $this->select_catalogo(cols:$cols,con_registros:$con_registros,id_selected:$id_selected,
            modelo: $modelo,  disabled: $disabled, label: 'Sucursal',required: true);
        if(errores::$error){
