...
class cat_sat_obj_impTest extends test {
    ...
    public function test_registro(): void 
    {
        ...
        $_GET['session_id'] = '1';
        $modelo = new cat_sat_obj_imp(link: $this->link);

        $del = (new \gamboamartin\cat_sat\tests\base_test())->del_cat_sat_obj_imp(link: $this->link);
        if(errores::$error){
        ...
        $alta = (new \gamboamartin\cat_sat\tests\base_test())->alta_cat_sat_obj_imp(link: $this->link);
        if(errores::$error){
