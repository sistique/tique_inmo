...
class cat_sat_productoTest extends test {
    ...
    public function test_registro(): void
    {
        ...
        $_GET['session_id'] = '1';
        $modelo = new cat_sat_producto(link: $this->link);

        $del = (new \gamboamartin\cat_sat\tests\base_test())->del_cat_sat_producto(link: $this->link);
        if(errores::$error){
        ...
        $alta = (new \gamboamartin\cat_sat\tests\base_test())->alta_cat_sat_producto(link: $this->link);
        if(errores::$error){
