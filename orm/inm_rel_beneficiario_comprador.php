...
class inm_rel_beneficiario_comprador extends _modelo_parent{
    public function __construct(PDO $link)
    {
        $tabla = 'inm_rel_beneficiario_comprador';
        $columnas = array($tabla=>false,'inm_comprador'=>$tabla,'inm_beneficiario'=>$tabla,
            'inm_tipo_beneficiario'=>'inm_beneficiario', 'inm_parentesco'=>'inm_beneficiario');

        $campos_obligatorios = array('inm_comprador_id','inm_beneficiario_id');

        $columnas_extra= array();

        $renombres = array();

        $atributos_criticos = array('inm_comprador_id','inm_beneficiario_id');
        ...
        $this->etiqueta = 'Relacion comprador beneficiario';
    }

    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {

        $keys = array('inm_beneficiario_id','inm_comprador_id');
        ...
        if(!$existe) {
            ...
            $data = $this->inm_rel_beneficiario_comprador_filtro(filtro: $filtro);
            if(errores::$error){
    ...
    private function inm_rel_beneficiario_comprador_filtro(array $filtro){
        $r_registro = $this->filtro_and(filtro: $filtro);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al obtener relacion', data: $r_registro);
        }

        $registro_puro = $this->registro(registro_id: $r_registro->registros[0]['inm_rel_beneficiario_comprador_id'],
