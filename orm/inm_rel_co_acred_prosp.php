...
class inm_rel_co_acred_prosp extends _modelo_parent{
    public function __construct(PDO $link)
    {
        $tabla = 'inm_rel_co_acred_prosp';
        $columnas = array($tabla=>false,'inm_prospecto'=>$tabla,'inm_co_acreditado'=>$tabla);

        $campos_obligatorios = array('inm_prospecto_id','inm_co_acreditado_id');

        $columnas_extra= array();

        $renombres = array();

        $atributos_criticos = array('inm_prospecto_id','inm_co_acreditado_id');
        ...
        $this->etiqueta = 'Relacion Prospecto Co acreditado';
    }

    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {

        $keys = array('inm_co_acreditado_id','inm_prospecto_id');
        ...
        if(!$existe) {
            ...
            $data = $this->inm_rel_co_acred_prosp_filtro(filtro: $filtro);
            if(errores::$error){
            ...
            $r_alta_bd = $this->data_result_transaccion(mensaje: 'Registro insertado con éxito', registro: $data->registro,
                registro_ejecutado: $this->registro, registro_id: $data->r_registro->registros[0]['inm_rel_co_acred_prosp_id'],
    ...
    private function inm_rel_co_acred_prosp_filtro(array $filtro){
        $r_registro = $this->filtro_and(filtro: $filtro);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al obtener relacion', data: $r_registro);
        }

        $registro_puro = $this->registro(registro_id: $r_registro->registros[0]['inm_rel_co_acred_prosp_id'],
