...
class inm_checada extends _modelo_parent{
    ...
    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {
        ...
        $in['llave'] = 'inm_status_asistencia.id';
        $in['values'] = array(2,3);

        $filtro_chec['inm_periodo_asistencia.id'] = $this->registro['inm_periodo_asistencia_id'];
        $r_checada_h = (new inm_checada(link:$this->link))->filtro_and(filtro: $filtro_chec, in: $in);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al obtener periodos', data: $r_checada_h);
        }

        if ($r_checada_h->n_registros > 0) {

            $hayRetardo = false;
            $hayInasistencia = false;
            foreach ($r_checada_h->registros as $checada) {
                if ($checada['inm_status_asistencia_id'] == 2) {
                    $hayRetardo = true;
                }

                if ($checada['inm_status_asistencia_id'] == 3) {
                    $hayInasistencia = true;
                }

                if ($hayRetardo && $hayInasistencia) {
                    break;
                }
            }
            ...
            if ($hayRetardo || $hayInasistencia) {
                ...
                $inicio = new DateTime('08:50:01');
                $fin = new DateTime('09:14:59');

                $timestamp_inicio = $inicio->getTimestamp();
                $timestamp_fin = $fin->getTimestamp();

                $timestamp_random = mt_rand($timestamp_inicio, $timestamp_fin);

                $hora = date('H:i:s', $timestamp_random);

                $this->registro['hora'] = $hora;
            }
        }
